...
print(mm.__file__)
files = ['r5.txt','r5.txt','ctd.txt','pol.txt','ppol.txt']
rigid = [[[]],[np.arange(12,250),np.arange(260,321),np.arange(324,452),np.arange(491,613),np.arange(908,991)],[[]],[np.arange(0,3898)],[np.arange(0,3898)]]
n_mol = [1,0,0,0,0]
print(f'PBC = {aminos.use_pbc}')
topology = app.Topology()
ba = 900
bb = 900
bc = 900
add(files,n_mol,box_a=ba,box_b=bb,box_c=bc,output='output.txt')
data = np.loadtxt('output.txt',skiprows=1)
i = 0
bonds = 0
r5_bonds = []
rigid_bodies = []
for protein in range(len(files)):
    atom_num = int(np.loadtxt(files[protein],skiprows=3,max_rows=1,usecols=[0]))
    rigid_region = np.concatenate((rigid[protein]))
    if n_mol[protein] > 0:
        for m in range(n_mol[protein]):
            r = False
...
print(topology)
system = mm.System()
box_vec_a = np.array([ba/10, 0, 0])*unit.nanometer
box_vec_b = np.array([0, bb/10, 0])*unit.nanometer
box_vec_c = np.array([0, 0, bc/10])*unit.nanometer
